...
大多数人使用 LLM 处理文档时，习惯把资料作为 RAG 的检索对象：每次提问时，模型从原始文档里找片段，再临时拼出答案。这种方式适合短期问答，但知识不会真正沉淀。下一次遇到复杂问题，LLM 仍然需要重新检索、重新拼接、重新推理。

LLM Wiki 的做法不同：它让 LLM 持续构建一个持久化 wiki。这个 wiki 是一组结构化、互相链接的 Markdown 文件，位于你和原始资料之间。每当你加入新资料，LLM 不只是建立索引，而是阅读、抽取、总结、对齐旧知识、维护交叉引用、更新相关页面、记录操作日志。
...
Codex 版在这个思路上做了进一步适配：它可以结合项目内 Python 虚拟环境做文档预处理，也可以调用 default agents 分批分析大量图片、截图课件和 PPT 页面。
